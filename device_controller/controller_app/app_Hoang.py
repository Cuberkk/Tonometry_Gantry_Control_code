...
class ControllerApp(tk.Tk):
    def __init__(self):
        ...
        # ------------------------------------------------------------------
        # Recorded-data directory (same hierarchy level as controller_app/)
        # Layout:  .../Tonometry_Control_Code-main/
        #              device_controller/
        #                  controller_app/   <- this file lives here
        #              recorded_data/        <- saved here
        # ------------------------------------------------------------------
        self._script_path           = os.path.dirname(os.path.abspath(__file__))   # controller_app/
        self._device_controller_dir = os.path.dirname(self._script_path)           # device_controller/
        self._project_root          = os.path.dirname(self._device_controller_dir) # Tonometry_Control_Code-main/

        if os.path.isdir(self._project_root):
            self.recorded_data_dir = os.path.join(self._project_root, "recorded_data")
        else:
            # Absolute fallback -- only used if __file__ cannot be resolved
            self.recorded_data_dir = (
                r"C:\Users\nmhoa\Downloads\Tonometry_Control_Code-main"
                r"\Tonometry_Control_Code-main\recorded_data"
            )

        try:
            os.makedirs(self.recorded_data_dir, exist_ok=True)
        except OSError as e:
            print(f"[WARNING] Could not create recorded_data dir: {e}")

        # New vars for the sequence-recording feature
        self.seq_record_active = tk.BooleanVar(value=True)    # "Record sequence" checkbox
        self.seq_record_name   = tk.StringVar(value="Record") # subject/folder name
        self.seq_record_number = tk.StringVar(value="1")      # test number

        # Unified start time (set when a sequence begins)
        self._adc_t0: float | None = None

        # In-memory buffer: each entry is [elapsed_s, pos_mm]
        self._recording_data_buffer: list = []

        # Flag to stop the recording background thread
        self._recording_thread_running = False
    ...
    def _sequence_panel(self):
        ...
        #   [v Record sequence]  [Name: ______]  [Number: __]  [Save data]
        row2 = ttk.Frame(f)
        row2.pack(fill=tk.X, padx=8, pady=(6, 8))

        ttk.Checkbutton(
            row2, text="Record sequence",
            variable=self.seq_record_active,
        ).pack(side=tk.LEFT, padx=(0, 16))

        ttk.Label(row2, text="Name:").pack(side=tk.LEFT)
        ttk.Entry(row2, textvariable=self.seq_record_name, width=14).pack(side=tk.LEFT, padx=(4, 10))

        ttk.Label(row2, text="Number:").pack(side=tk.LEFT)
        ttk.Entry(row2, textvariable=self.seq_record_number, width=6).pack(side=tk.LEFT, padx=(4, 16))

        # Save button -- disabled until a sequence finishes with recorded data
        self.btn_seq_save = ttk.Button(
            row2, text="Save data",
            command=self._on_save_csv,
            state=tk.DISABLED,
        )
        self.btn_seq_save.pack(side=tk.LEFT)
    ...
    def start_sequence(self):
        ...
        # --- recording setup ---
        # Clear previous buffer and plot data for a fresh run
        self._recording_data_buffer = []
        self._adc_t0 = time.monotonic()

        # Start background recording thread if checkbox is ticked
        if self.seq_record_active.get():
            self._recording_thread_running = True
            self._recording_thread = threading.Thread(
                target=self._recording_data_task,
                daemon=True,
            )
            self._recording_thread.start()

        # --- existing logging / sequence launch ---
        ...
        rec_txt = "ON" if self.seq_record_active.get() else "OFF"
        self.lbl_status.config(text=f"Weights sequence running — recording {rec_txt}")
    ...
    def _sequence_finished(self, success: bool, completed: int, requested: int):
        ...
        # Stop the recording thread if it is still alive
        self._recording_thread_running = False
        if hasattr(self, "_recording_thread") and self._recording_thread is not None:
            self._recording_thread.join(timeout=1.0)
            self._recording_thread = None
    ...
    def _update_sequence_controls(self):
        ...
        if btn_stop is not None:
            if self.sequence_running:
                ...
                btn_stop.state(["disabled"])
        # Save is available when the sequence has stopped AND there is buffered data
        if btn_save is not None:
            has_data = bool(self._recording_data_buffer)
            save_ok  = (not self.sequence_running) and has_data
            btn_save.config(state=tk.NORMAL if save_ok else tk.DISABLED)



    # ------------------------------------------------------------------
    # Sequence data recording (real ADC data -> in-memory buffer -> CSV)
    # ------------------------------------------------------------------
    def _recording_data_task(self):
        """
        ...
        while self._recording_thread_running and self.sequence_running:
            ...
            # Read real LVDT position from the ADC object
            pos_mm = self.adc.latest_pos if (self.adc and self.adc.enabled) else 0.0
            self._recording_data_buffer.append([elapsed, pos_mm])
            time.sleep(INTERVAL_S)

        print(f"[RECORD] Recording thread stopped. "
              f"Captured {len(self._recording_data_buffer)} samples.")

    def _on_save_csv(self):
        """
        Triggered by the Save data button.
        Disables the button immediately to prevent double-clicks, then
        writes the in-memory buffer to disk in a background thread so
        the GUI stays responsive.
        """
        self.btn_seq_save.config(state=tk.DISABLED)

        data_to_save = list(self._recording_data_buffer)
        if not data_to_save:
            self.lbl_status.config(
                text="Nothing to save (no recorded data).", foreground="orange"
            )
            return

        threading.Thread(
            target=self._write_csv_file,
            args=(data_to_save,),
            daemon=True,
        ).start()

    def _write_csv_file(self, data: list):
        """
        Background-thread worker.  Writes data to:
            recorded_data/<Name>/test_number_<Number>.csv

        The <Name> subfolder is created if it does not exist.
        An existing file with the same name is overwritten.
        """
        try:
            folder_name = self.seq_record_name.get().strip().replace(" ", "_") or "Record"
            file_num    = self.seq_record_number.get().strip().replace(" ", "_") or "1"
            filename    = f"test_number_{file_num}.csv"

            save_dir  = os.path.join(self.recorded_data_dir, folder_name)
            full_path = os.path.join(save_dir, filename)

            os.makedirs(save_dir, exist_ok=True)

            print(f"[RECORD] Writing CSV -> {full_path}")

            with open(full_path, mode="w", newline="") as fh:
                writer = csv.writer(fh)
                writer.writerow(["Time (s)", "Position (mm)"])
                for row in data:
                    writer.writerow([f"{row[0]:.2f}", f"{row[1]:.4f}"])

            display = f"{folder_name}/{filename}"
            ...
            self.after(0, lambda: self.lbl_status.config(
                text=err_msg, foreground="#c62828"
            ))
